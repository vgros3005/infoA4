...
@section('title', __('Modifier') . ' — ' . $oUser->full_name)
@section('page-title', __('Modifier l\'utilisateur'))
...
@section('content')
<form method="POST" action="{{ route('admin.users.update', $oUser) }}">
    @csrf
    @method('PUT')
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <input type="text" id="first_name" name="first_name"
                                   ...
                                   value="{{ old('first_name', $oUser->first_name) }}" required>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" id="last_name" name="last_name"
                                   ...
                                   value="{{ old('last_name', $oUser->last_name) }}" required>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="email" id="email" name="email"
                                   ...
                                   value="{{ old('email', $oUser->email) }}" required>
                        ...
                        <div class="col-md-6 d-flex align-items-end pb-1">
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" id="is_active"
                                       ...
                                       {{ old('is_active', $oUser->is_active ?? true) ? 'checked' : '' }}>
                        ...
                        <div class="col-12">
                            <hr class="my-0">
                            <small class="text-muted">{{ __('Laissez vide pour conserver le mot de passe actuel') }}</small>
                        </div>
                        <div class="col-md-6">
                            <label for="password" class="form-label fw-medium">{{ __('Nouveau mot de passe') }}</label>
                        ...
                        <div class="col-md-6">
                            <label for="password_confirmation" class="form-label fw-medium">{{ __('Confirmation') }}</label>
                ...
                <div class="card-body">
                    <div id="teamRolesContainer">
                        ...
                        @forelse($oUser->teamUserRoles as $oTUR)
                            <div class="team-role-row row g-2 mb-2" data-index="{{ $iIdx }}">
                                <div class="col-6">
                                    <select name="team_roles[{{ $iIdx }}][team_id]" class="form-select form-select-sm">
                                        <option value="">{{ __('Équipe…') }}</option>
                                        @foreach($aTeams ?? [] as $oTeam)
                                            <option value="{{ $oTeam->id }}" {{ $oTUR->team_id == $oTeam->id ? 'selected' : '' }}>
                                                {{ $oTeam->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-5">
                                    <select name="team_roles[{{ $iIdx }}][role_id]" class="form-select form-select-sm">
...
<script>
(function () {
    let iRowIndex = {{ $oUser->teamUserRoles->count() ?: 1 }};
