...
@section('title', __('ui.new_request'))
@section('page-title', __('ui.new_request'))

@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('requests.index') }}">{{ __('ui.requests') }}</a></li>
    <li class="breadcrumb-item active">{{ __('ui.create') }}</li>
@endsection
    ...
    <div class="row g-3">
            ...
            <div class="card border-0 shadow-sm mb-3">
                <div class="card-header bg-transparent border-0">
                    <h5 class="card-title mb-0">
                        <i class="bi bi-info-circle me-2 text-primary"></i>{{ __('ui.general_info') }}
                    </h5>
                </div>
                <div class="card-body">
                    <div class="mb-3">
                        <label for="title" class="form-label fw-medium">
                            {{ __('ui.request_title') }} <span class="text-danger">*</span>
                        </label>
                    ...
                    <div class="mb-3">
                        <label for="description" class="form-label fw-medium">
                            {{ __('ui.request_description') }} <span class="text-danger">*</span>
                        </label>
                    ...
                    <div class="mb-3">
                        ...
                        @error('content')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        <div class="form-text">{{ __('ui.content_hint') }}</div>
                    </div>
                </div>
            </div>

            @can('seeInternalNotes', \App\Models\RequestA4::class)
            <div class="card border-0 shadow-sm mb-3 border-start border-warning border-3">
                <div class="card-header bg-transparent border-0">
                    <h5 class="card-title mb-0">
                        <i class="bi bi-lock me-2 text-warning"></i>{{ __('ui.internal_notes') }}
                        <span class="badge bg-warning text-dark ms-2 small">{{ __('ui.admin_cp') }}</span>
                    </h5>
                </div>
                <div class="card-body pt-0">
                    <textarea id="internal_notes" name="internal_notes" rows="3"
                              class="form-control @error('internal_notes') is-invalid @enderror"
                              placeholder="{{ __('ui.internal_notes_hint') }}">{{ old('internal_notes') }}</textarea>
        ...
        <div class="col-12 col-lg-4">
            <div class="card border-0 shadow-sm mb-3">
                <div class="card-header bg-transparent border-0">
                    <h5 class="card-title mb-0">
                        <i class="bi bi-tags me-2 text-secondary"></i>{{ __('ui.classification') }}
                    </h5>
                </div>
                <div class="card-body">
                    <div class="mb-3">
                        <label for="request_type_id" class="form-label fw-medium">
                            {{ __('ui.request_type') }} <span class="text-danger">*</span>
                        </label>
                        <select id="request_type_id" name="request_type_id"
                            ...
                            @foreach($aRequestTypes as $oType)
                                <option value="{{ $oType->id }}" {{ old('request_type_id') == $oType->id ? 'selected' : '' }}>
                                    {{ $oType->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        <label for="priority_id" class="form-label fw-medium">
                            {{ __('ui.priority') }} <span class="text-danger">*</span>
                        </label>
                        <select id="priority_id" name="priority_id"
                            ...
                            @foreach($aPriorities as $oPriority)
                                <option value="{{ $oPriority->id }}"
                                        ...
                                        {{ old('priority_id') == $oPriority->id ? 'selected' : '' }}>
                                    {{ $oPriority->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3" id="justificationBlock" style="display: none;">
                        <label for="priority_justification" class="form-label fw-medium">
                            {{ __('ui.justification') }} <span class="text-danger">*</span>
                        </label>
                        <textarea id="priority_justification" name="priority_justification" rows="3"
                                  ...
                                  placeholder="{{ __('ui.justification_placeholder') }}">{{ old('priority_justification') }}</textarea>
            ...
            <div class="card border-0 shadow-sm mb-3">
                <div class="card-header bg-transparent border-0">
                    <h5 class="card-title mb-0">
                        <i class="bi bi-building me-2 text-secondary"></i>{{ __('ui.perimeter') }}
                    </h5>
                </div>
                <div class="card-body">
                    <div class="mb-3">
                        <label class="form-label fw-medium">{{ __('ui.scope_companies') }} <span class="text-danger">*</span></label>
                            ...
                            @foreach($aCompanies as $oCompany)
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox"
                                           ...
                                           {{ in_array($oCompany->id, (array) old('company_ids', [])) ? 'checked' : '' }}>
                    ...
                    <div class="mb-0">
                        <label class="form-label fw-medium">{{ __('ui.scope_softwares') }}</label>
                        <div class="border rounded p-2" style="max-height: 150px; overflow-y: auto;">
                            @foreach($aSoftwares as $oSoftware)
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox"
                                           ...
                                           {{ in_array($oSoftware->id, (array) old('software_ids', [])) ? 'checked' : '' }}>
            ...
            <div class="card border-0 shadow-sm mb-3">
                <div class="card-header bg-transparent border-0">
                    <h5 class="card-title mb-0">
                        <i class="bi bi-calendar3 me-2 text-secondary"></i>{{ __('ui.dates') }}
                    </h5>
                </div>
                <div class="card-body">
                    <div class="mb-3">
                        <label class="form-label fw-medium">{{ __('ui.requested_date') }}</label>
                        <p class="form-control-plaintext">{{ now()->format('d/m/Y') }}</p>
                        <small class="text-muted">{{ __('ui.requested_date_auto') }}</small>
                    </div>
                    <div class="mb-0">
                        <label for="desired_date" class="form-label fw-medium">{{ __('ui.desired_date') }}</label>
            ...
            <div class="d-grid gap-2">
                <button type="submit" class="btn btn-primary">
                    <i class="bi bi-save me-1"></i>{{ __('ui.save_request') }}
                </button>
